...
export const ToDo = ({ activity, priority, date, handClick, classe, id, done, setList, setMemory }) => {
    ...
    rounded-3xl ${done?"":"line-through" } ${priority === 'low' ? 'bg-green-400' : priority == 'Medium' ?  'bg-amber-300' : 'bg-red-600' }`}>
        ...
        <button onClick={() => { handClick(id) }} className='bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-l'>ELIMINAR</button>
        <button onClick={()=>{handRealizadas(done)}} className='bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-r'>REALIZADA</button>
        </div>
    </div>
